...
const UpdateWeight = (props: any) => {
  ...
  useEffect(() => {
    let totalWeight = 0;
    props.newWeightOwner.forEach((item: any) => { totalWeight += parseInt(item?.weight) });
    props?.newWeightOwner.map((item: any, index: number) => {
      if (ErrorWeightMessage(props.newWeightOwner, index) === ''
        && props.newWeightOwner[index].weight !== undefined
      ) {
        setSave(true);
      }
      else {
  ...
  }, [props?.newWeightOwner])


  const NavigateConfirm = () => {
    if (save) {
      router.push({
        pathname: "/Wallet/Setting/UpdateOwnerWeightConfirm",
        query: {
          newWeightOwner: JSON.stringify(props.newWeightOwner)
        }
      })
    }
    else {
              ...
              return (
                ...
                >
                  ...
                  <CssTextField
                    value={item?.weight ?? ''}
                    type={"number"}
                    onChange={(e) => { props.ChangeOwnerWeight(index, e.target.value) }}
                    error={ErrorWeightMessage(props?.newWeightOwner, index) === '' ? false : true}
                    helperText={ErrorWeightMessage(props?.newWeightOwner, index)}
                    sx={{ width: "100%", padding: "0", marginTop: ".625rem" }}
                    InputProps={{ style: { height: "40px", fontSize: ".875rem", backgroundColor: "#FFFFFF" } }}
