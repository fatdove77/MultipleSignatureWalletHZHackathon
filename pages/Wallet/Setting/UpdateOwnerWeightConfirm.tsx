...
export default function UpdateOwnerWeightConfirm(props: any) {
  const router = useRouter();
  console.log(JSON.parse((router?.query?.newWeightOwner) as string ?? "[]"));
  ...
  const NavigateUpdateProcess = useThrottle(async () => {
    ...
    let temp_newWeightOwner = JSON.parse((router?.query?.newWeightOwner) as string ?? "[]");
    //修改transferInfo!!!!!!!!!!!!!!
    let temp_transferInfo = {
      walletInfo: walletInfo,
      newWeightOwner: temp_newWeightOwner,
      createTime: time,
      nonce: nonce ?? 0
    }
    ...
    if (temp_transferInfo&&raw) {
      await CheckOwner(temp_transferInfo).then(async(res: any) => {
        if (res) {
          console.log(walletInfo?.address, nonce ?? 0, "修改权重值", JSON.stringify(temp_transferInfo));
          await CreateNewTransaction(walletInfo?.address, nonce ?? 0, "修改权重值", JSON.stringify(temp_transferInfo))
          //我需要把所有都存在缓存中
          router.push('UpdateOwnerWeightProcess')
        }
        else {
          toast.error("用户不在多签内")
        }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: "93.5vh" }}>
      <Grid
        ...
        container
        alignItems={'center'}
      ><LeftOutlined style={{ marginRight: ".3rem" }} />修改成员权重值确认</Grid>
      <Grid
        container
        item
        sx={{
      ...
      >
        ...
        >
          ...
          >
            {
              JSON.parse((router?.query?.newWeightOwner) as string ?? "[]")
