...
export default function AddOwnerConfirm(props: any) {
  const router = useRouter();
  console.log(JSON.parse((router?.query?.updateOwnerArr) as string ?? "[]"));
  ...
  const NavigateUpdateProcess= useThrottle(async () => {
    ...
    let temp_updateOwnerArr = JSON.parse((router?.query?.updateOwnerArr) as string ?? "[]");
    let first = temp_updateOwnerArr.pop();
    first.img = ethereumAddressImage(first?.address);
    temp_updateOwnerArr.unshift(first);
    //修改transferInfo!!!!!!!!!!!!!!
    let temp_transferInfo = {
      walletInfo: walletInfo,
      updateOwnerArr: temp_updateOwnerArr,
      createTime: time,
      nonce: nonce ?? 0
    }
    ...
    if (temp_transferInfo&&raw) {
      await CheckOwner(temp_transferInfo).then(async(res: any) => {
        if (res) {
          console.log(walletInfo?.address, nonce ?? 0, "添加成员", JSON.stringify(temp_transferInfo));
          await CreateNewTransaction(walletInfo?.address, nonce ?? 0, "添加成员", JSON.stringify(temp_transferInfo))
          //我需要把所有都存在缓存中
          router.push('AddOwnerProcess')
        }
        else {
          toast.error("用户不在多签内")
        }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: "93.5vh" }}>
      <Grid
        ...
        container
        alignItems={'center'}
      ><LeftOutlined style={{ marginRight: ".3rem" }} />添加成员确认</Grid>
      <Grid
        container
        item
        sx={{
      ...
      >
        ...
        >
          ...
          >
            {
              JSON.parse((router?.query?.updateOwnerArr) as string ?? "[]")
                .slice(walletInfo?.data?.ownerArr.length)
