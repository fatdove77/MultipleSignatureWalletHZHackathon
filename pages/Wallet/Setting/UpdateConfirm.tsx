...
export default function UpdateConfirm(props: any) {
  ...
  const NavigateUpdateProcess = useThrottle(async() => {
    ...
    let temp_transferInfo = {
      ...
      newWeight: parseInt((router.query?.newWeight)as string) ?? undefined,
      newDoor: parseInt((router.query?.newDoor)as string) ?? undefined,
      createTime:time,
      nonce:nonce??0
    }
    ...
    if (temp_transferInfo&&raw) {
      await CheckOwner(temp_transferInfo).then(async(res: any) => {
        if (res) {
          console.log(walletInfo?.address, nonce??0, walletInfo?.data?.signType===0?"修改门限值":"修改权重门限值", JSON.stringify(temp_transferInfo));
          await CreateNewTransaction(walletInfo?.address, nonce??0, walletInfo?.data?.signType===0?"修改门限值":"修改权重门限值", JSON.stringify(temp_transferInfo))
          //我需要把所有都存在缓存中
          router.push('UpdateThresholdProcess')
        }
        else {
          toast.error("用户不在多签内")
        }
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: "93.5vh" }}>
      <Grid
        ...
        container
        alignItems={'center'}
      ><LeftOutlined style={{ marginRight: ".3rem" }} />更改门槛确认</Grid>
      <Grid
        container
        item
        sx={{
            ...
            <Grid
              ...
              {/* 根据门槛种类 */}
              {
               walletInfo?.data?.signType === 0
                  ...
                  >
                    {`${router?.query?.newDoor} out of ${walletInfo?.data?.ownerArr?.length} owner(s)`}
                  </Typography>
                  : <Typography
                    sx={{
                      color: "#000000",
