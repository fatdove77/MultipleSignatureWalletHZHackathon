...
export default function WithdrawConfirm(props: any) {
  ...
  const GetRawData = async () => {
    const transaction = {
      ...
      nonce: JSON.parse(router.query?.tradeInfo as string ?? "{}")?.Nonce??"", // 交易序号
    };
  ...
  const NavigateUpdateProcess=useThrottle(async () => {
    ...
    let temp_transferInfo = {
      ...
      createTime: time,
      nonce: JSON.parse(router.query?.tradeInfo as string ?? "{}")?.Nonce??""
    }
    ...
    if (temp_transferInfo&&raw) {
      await CheckOwner(temp_transferInfo).then(async(res: any) => {
        if (res) {
          console.log(walletInfo?.address, JSON.parse(router.query?.tradeInfo as string ?? "{}")?.Nonce??"");
          await CancelTrade(walletInfo?.address, JSON.parse(router.query?.tradeInfo as string ?? "{}")?.Nonce??"")
          //我需要把所有都存在缓存中
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: "93.5vh" }}>
      <Grid
        ...
        container
        alignItems={'center'}
      ><LeftOutlined style={{ marginRight: ".3rem" }} />撤销事务</Grid>
      <Grid
        container
        item
        sx={{
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {JSON.parse(router.query?.tradeInfo as string ?? "{}")?.TransactionType??""}
              </Typography>
            </Grid>
            <Grid
              container
            ...
            >
              ...
              >
                {JSON.parse(router.query?.tradeInfo as string ?? "{}")?.Nonce??""}
              </Typography>
            </Grid>
          </Grid>
