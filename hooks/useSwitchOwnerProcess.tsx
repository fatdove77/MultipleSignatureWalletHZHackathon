...
export const useSwitchOwnerProcess = () => {
  ...
  const OwnerSign = async () => {
    ...
    let swapOwnerTypes: any;
    let swapOwnerMessage: any;
    if (transferInfo?.walletInfo?.data?.signType === 0) {
      swapOwnerTypes = {
        SwapOwner: [
      ...
      swapOwnerMessage = {
        oldOwner: transferInfo?.oldAddress,
        newOwner: transferInfo?.switchAddress,
      ...
      // 删除后的对应权重值
      //存储钱包成员
      let temp_transferInfo_address = JSON.parse(JSON.stringify(transferInfo?.walletInfo?.data?.ownerArr));
      let temp_transferInfo_weight = JSON.parse(JSON.stringify(transferInfo?.walletInfo?.data?.ownerArr));
      // 替换后的地址数组
      ...
      owners.map((item:any,index:number)=>{
        if(item===transferInfo?.oldAddress){
          owners[index] = transferInfo?.switchAddress
        }
      })
      // owners[transferInfo?.switchIndex] = transferInfo?.switchAddress;
      swapOwnerMessage = {
        oldOwner: transferInfo?.oldAddress,
        newOwner: transferInfo?.switchAddress,
        owners: owners,
    ...
    //拿到打包后的签名 
    console.log("📝📝签名类型和签名信息", swapOwnerTypes, swapOwnerMessage);
    const res = await GetSignature(provider, account, domain, swapOwnerTypes, swapOwnerMessage);
    ...
    walletArr.map(async (item: any, index: number) => {
      ...
      if (item?.address === transferInfo?.walletInfo?.address) {
        ...
        temp.map((item:any,index:number)=>{
          if(item.address===transferInfo?.oldAddress){
            item.address = transferInfo?.switchAddress;
            item.name = transferInfo?.switchName;
          }
        })
        // temp[transferInfo?.switchIndex].address = transferInfo?.switchAddress
        // temp[transferInfo?.switchIndex].name = transferInfo?.switchName;
        item.data.ownerArr=temp;  //更新原有钱包
        try {
          await ReplaceOwner(item?.address,transferInfo?.oldAddress,transferInfo?.switchAddress)
        } catch (error) {
          console.log("替换地址出错");
          console.log(error);
        }
  ...
  const CallDoorTrans = async (concatSignature: string,) => {
    ...
    try {
      console.log(
        "调用合约的参数",
        pre,
        transferInfo?.oldAddress,
        transferInfo?.switchAddress,
        concatSignature
      );
      ...
      gasPrice = 100 * 1e9;
      res = await DoorContract?.['swapSigOwner'](...[
        pre,
        transferInfo?.oldAddress,
        transferInfo?.switchAddress,
        concatSignature
  ...
  const CallWeightTrans = async (concatSignature: string,) => {
    ...
    try {
      ...
      gasLimit = gas?.toNumber();
      gasPrice = 100 * 1e9;
      res = await WeightContract?.['swapSigOwner'](...[
        pre,
        transferInfo?.oldAddress,
        transferInfo?.switchAddress,
        owners,
