...
export const useUpdateThreshold = () => {
  ...
  const OwnerSign = async () => {
    ...
    let changeThresholdTypes: any;
    let changeThresholdMessage: any;
    if (transferInfo?.walletInfo?.data?.signType === 0) {
      changeThresholdTypes = {
        ChangeThreshold: [
          { name: "threshold", type: "uint256" },
          { name: "nonce", type: "uint256" },
        ],
      };
      changeThresholdMessage = {
        threshold: transferInfo?.newDoor,
        nonce: transferInfo?.nonce
      }
    }
    else {
      changeThresholdTypes = {
        ChangeWeightThreshold: [
          { name: "weightThreshold", type: "uint256" },
          { name: "nonce", type: "uint256" },
        ],
      };
      changeThresholdMessage = {
        weightThreshold: transferInfo?.newWeight,
        nonce: transferInfo?.nonce
      }
    }
    //拿到打包后的签名 
    const res = await GetSignature(provider, account, domain, changeThresholdTypes, changeThresholdMessage);
  ...
  const UpdateWalletInfo = () => {
    ...
    walletArr.map((item: any, index: number) => {
      if (item.address === transferInfo?.walletInfo?.address) {
        if (item?.data?.signType === 0) {
          walletArr[index].data.door = transferInfo?.newDoor;
          console.log("🐔🐔🐔", walletArr);
        }
        else {
          walletArr[index].data.weight = transferInfo?.newWeight;
        }
  ...
  const CallDoorTrans = async (concatSignature: string,) => {
    ...
    //调合约/////////////////////////
    try {
      console.log("调用合约的参数", transferInfo?.newDoor, concatSignature);
      gas = await DoorContract?.estimateGas
        ?.['alterThreshold'](...[
          transferInfo?.newDoor.toString(),
          concatSignature
        ])
      console.log("gas值为：" + gas?.toNumber());
      gasLimit = gas?.toNumber();
      gasPrice = 100 * 1e9;
      res = await DoorContract?.['alterThreshold'](...[
        transferInfo?.newDoor.toString(),
        concatSignature
      ], { gasLimit, gasPrice })  //,
      Message(
  ...
  const CallWeightTrans = async (concatSignature: string,) => {
    ...
    try {
      console.log("调用合约的参数", transferInfo?.newWeight, concatSignature, length);
      gas = await WeightContract?.estimateGas
        ?.['alterWeightThreshold'](...[
          transferInfo?.newWeight,
          concatSignature,
          length
        ])
      console.log("gas值为：" + gas?.toNumber());
      gasLimit = gas?.toNumber();
      gasPrice = 100 * 1e9;
      res = await WeightContract?.['alterWeightThreshold'](...[
        transferInfo?.newWeight,
        concatSignature,
        length
      ], { gasLimit, gasPrice })  //,
