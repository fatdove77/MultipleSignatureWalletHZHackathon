...
export const useWithdrawProcess = () => {
  ...
  const OwnerSign = async () => {
    ...
    //拿到打包后的签名 
    const res = await GetSignature(provider, account, domain, cancelTypes, cancelMessage);
  ...
  const CallDoorTrans = async (concatSignature: string,) => {
    ...
    //调合约/////////////////////////
    try {
      console.log("调用合约的参数", concatSignature);
  ...
  const CallWeightTrans = async (concatSignature: string,) => {
    ...
    //调合约/////////////////////////
    try {
      console.log("调用合约的参数", concatSignature, length);
      ...
      gasLimit = gas?.toNumber();
      gasPrice = 100 * 1e9;
      res = await WeightContract?.['cancel'](...[
        concatSignature,
        length
      ], { gasLimit, gasPrice })  //,
      Message(
