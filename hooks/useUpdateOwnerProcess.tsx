...
export const useUpdateOwnerProcess = () => {
  ...
  const OwnerSign = async () => {
    ...
    let changeWeightTypes: any;
    let changeWeightMessage: any;
    changeWeightTypes = {
      ChangeWeight: [
    ...
    //用户权重数组
    let temp_weights = transferInfo?.newWeightOwner.map((item: any) => item?.weight);
    //用户地址数组
    let temp_owners = transferInfo?.newWeightOwner.map((item: any) => item?.address);
    changeWeightMessage = {
      owners: temp_owners,
    ...
    //拿到打包后的签名 
    console.log("📝📝签名类型和签名信息", changeWeightTypes, changeWeightMessage);
    const res = await GetSignature(provider, account, domain, changeWeightTypes, changeWeightMessage);
  ...
  const UpdateWalletInfo = () => {
    ...
    walletArr.map(async (item: any, index: number) => {
      ...
      if (item?.address === transferInfo?.walletInfo?.address) {
        //更新原有用户的权重
        transferInfo?.newWeightOwner.map((owner: any, i: number) => {
  ...
  const CallWeightTrans = async (concatSignature: string,) => {
    ...
    try {
      //更新后的地址和权重数组
      let weights = transferInfo?.newWeightOwner.map((item: any) => item?.weight);
      let owners = transferInfo?.newWeightOwner.map((item: any) => item?.address);

      console.log("调用合约的参数", owners, weights, concatSignature, length);
      ...
      gasPrice = 100 * 1e9;
      res = await WeightContract?.['alterWeight'](...[
        owners,
        weights,
        concatSignature,
        length
